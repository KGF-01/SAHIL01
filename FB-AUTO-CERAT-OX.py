#---------------------------------------------
#-------SEND :- https://t.me/+LRlET_sIrUcxMTk1
#OX CYBER TEAM
#---------------------------------------------
...
{W}    F{G}B{W} ╹ ╹┗━┛ ╹ ┗━┛   ┗━╸╹┗╸┗━╸╹ ╹ ╹ ┗━╸ 1.3
{W}──────────────────────────────────────────────────
{xd} TELIGERM  {R}:{W} https://t.me/+LRlETCreatexMTk1
{xd} TOOLS {R}:{W} Facebook Ids Auto Create
{xd} SEND BY : {W} KALYAN KING
{W}──────────────────────────────────────────────────''')
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            print(f'''\r{xd}-{W}[{G}OX-OK{W}]{G} {uid} | KALYAN3727 ''')
            print(f'''\r{xd}-{cookie} ''');linex()
            open('/sdcard/OX-AUTO-CRATE-OK-ID.txt', 'a').write(uid + '|KALYAN3727|' + cookie + '\n')
            oks.append(uid)
    except Exception as e:
        pass
